 ...
 * Pure JVM — no external dependencies beyond javax.crypto.
 */
object HkdfSha256 {
     ...
     *
     * @param ikm   input keying material (e.g. raw ECDH shared secret)
     * @param salt  optional salt (use non-secret, context-binding value)
     * @param info  context/application-specific info string
     * @param length desired output length in bytes (1..8160)
     * @return derived key material — caller must zero when done
     */
    fun derive(
        ikm: ByteArray,
        salt: ByteArray,
