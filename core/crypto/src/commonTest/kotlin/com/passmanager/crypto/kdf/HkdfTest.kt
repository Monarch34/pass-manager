...
class HkdfTest {
    ...
    fun `RFC 5869 test case 1`() {
        assertEquals(
            ...
            hkdfSha256(
                inputKeyMaterial = Secret.of(hex("0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b0b")),
    ...
    fun `different info produces unrelated output`() {
        val secret = Secret.of(hex("000102030405060708090a0b0c0d0e0f101112131415161718191a1b1c1d1e1f"))
        val salt = hex("a1a2a3a4")
        val content = hkdfSha256(secret, salt, "content".encodeToByteArray(), 32).toByteArray().toHex()
        val index = hkdfSha256(secret, salt, "index".encodeToByteArray(), 32).toByteArray().toHex()
    ...
    fun `lengths across the block boundaries are prefixes of one another`() {
        ...
        val longest = hkdfSha256(secret, ByteArray(0), ByteArray(0), 255 * 32).toByteArray().toHex()
        for (length in intArrayOf(1, 31, 32, 33, 64, 65, 255 * 32)) {
            ...
            assertEquals(longest.substring(0, length * 2), derived.toByteArray().toHex(), "length $length")
        }
    }

    @Test
    fun `rejects lengths outside the specified range`() {
        val secret = Secret.of(hex("00112233"))
